...
Build the `/editor` home screen and add project dialogs/sidebar actions. Wire project data and project mutations to the real backend project API.
...
Use real project data from the backend project API.

Create uses `POST /api/projects` and navigates to the new workspace.
Rename uses `PATCH /api/projects/[id]` and refreshes on success.
Delete uses `DELETE /api/projects/[id]` and redirects to `/editor` when deleting the active workspace.
